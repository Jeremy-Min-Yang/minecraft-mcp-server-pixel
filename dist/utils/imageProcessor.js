...
function getColorDistance(r1, g1, b1, r2, g2, b2, options = {}) {
    const { redWeight = 0.299, greenWeight = 0.587, blueWeight = 0.114 } = options;
    // Using weighted Euclidean distance to account for human perception
    return Math.sqrt(redWeight * Math.pow(r1 - r2, 2) +
        greenWeight * Math.pow(g1 - g2, 2) +
        blueWeight * Math.pow(b1 - b2, 2));
}
// Find the closest Minecraft block to a given RGB color
function findClosestBlockColor(r, g, b, a = 255) {
...
function processJimpImage(image, maxHeight) {
...
export async function processLocalImage(filePath, maxHeight = 50, options = {}) {
    try {
...
export async function processImageUrl(imageUrl, maxHeight = 50, options = {}) {
    try {
...
export async function processBase64Image(base64String, maxHeight = 50, options = {}) {
    try {
...
export async function processImageBuffer(buffer, maxHeight = 50, options = {}) {
    try {
        let image = await Jimp.read(buffer);
        // Apply dithering if requested
        if (options.dithering) {
...
export function savePixelArtBlueprintToFile(blockArray, filePath) {
    const blueprint = {
        pixels: blockArray,
...
export function getBlockCount(blockArray) {
    const blockCounts = {};
...
export function formatBlockRequirements(blockCounts) {
